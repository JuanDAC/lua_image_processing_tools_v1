...
Promise, async, await = (() -> 
	...
	class Promise
		...
		@resolve=(val) ->
			Promise (res, rej) -> res val
		
		-- returns a Promise that rejects with reason
		@reject=(reason) ->
			Promise (res, rej) -> rej reason
...
class ConfigurationGeneral
	...
	new: () => 
		@main = Dialog("configuration general")
		@main_rer!
		--@selected_masks_rer!

	main_rer: () => 
		@section_selection_of_settings(dialog: @main)
		@show(dialog: @main)

	selected_masks_rer: () =>
		@selected_masks = Dialog("Selected masks")
		@section_selected_masks(dialog: @selected_masks)
		@selected_masks.bounds = Rectangle(10, 30, @selected_masks.bounds.width, @selected_masks.bounds.height)
		@show(dialog: @selected_masks)
	
	show: (props) => 
		props.dialog\show(wait: false)

	section_selection_of_settings: (props) =>
		

	section_selected_masks: (props) =>



class ConfigurationMask
	new: (props) => 
		@mask_filename = props.filename
		@mask_id = props.id
		@main = Dialog("configuration of the mask ")
		@main_rer!
		--@selected_masks_rer!

	main_rer: () => 
		@section_selection_of_mask(dialog: @main)
		@show(dialog: @main)
	...
	show: (props) => 
		props.dialog\show(wait: false)
	section_selection_of_mask: (props) =>
		...
		props.dialog\button {
			...
			text: "Siguiente",
			onclick: () -> 
				if props.dialog.data.color_mask and props.dialog.data.selected_decity
					Data\set_masks({
						{
							id: @mask_id,
					...
					app.alert('Los datos no consistentes')
				--@selected_masks\close!
				--@main\close!
				-- comprueva data.ok
				-- envia informacion recolectada 
				-- cierra de tener la informacion corecta 
...
class SelectorMask 
	...
	section_selection_of_mask: (props) =>
		...
		props.dialog\combobox {
			...
			options: Array\map(app.sprites, (item) -> 
				item.filename if string.find(item.filename, 'mask') != nil
			)
		}
		props.dialog\button {
			id: "add_of_mask",
			text: "Agregar",
			onclick: () ->
				if not Array\includes(@masks, props.dialog.data.selection_of_mask)
					@masks = Array\concat(@masks, {props.dialog.data.selection_of_mask})
					@selected_masks\close!
					@selected_masks_rer!
		}
		props.dialog\newrow!
		...
		props.dialog\button {
			...
			onclick: () -> 
				@selected_masks\close!
				@main\close!
				Array\map(@masks, (mask, id) ->
					ConfigurationMask(filename: mask, id: id)
				)
	...
	section_selected_masks: (props) =>
		...
		Array\map(@masks, (item, index) -> 
			props.dialog\label {
				id: "selected_masks_#{index}",
				text: item
			}
			props.dialog\button {
				id: "remove_mask_#{index}",
				text: "Quitar",
				onclick: () -> 
					Array\remove(@masks, (value) -> 
						return item == value
					)
					@selected_masks\close!
					@selected_masks_rer!
